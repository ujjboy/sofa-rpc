...
public class Jsr303Validator implements Validator {
    ...
    /**
     * Instantiates a new Jsr 303 validator.
     *
     * @param className       the class name
     * @param customValidator the custom validator
     */
    @SuppressWarnings("unchecked")
    public Jsr303Validator(String className, String customValidator) {
        this.className = className;
        this.clazz = ClassUtils.forName(className);
        javax.validation.ValidatorFactory factory;
        if (customValidator != null && customValidator.length() > 0) {
            factory = Validation.byProvider(ClassUtils.forName(customValidator))
    ...
    private static Object getMethodParameterBean(Class<?> clazz, Method method, Object[] args) {
        ...
        try {
            ...
            try {
                parameterClass = ClassUtils.forName(parameterClassName);
            } catch (Exception e) {
